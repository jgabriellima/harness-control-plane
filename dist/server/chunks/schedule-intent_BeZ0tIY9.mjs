const SCHEDULE_PHRASE = /\b(every\s+(day|morning|evening|night|hour|week|monday|tuesday|wednesday|thursday|friday|saturday|sunday|weekday|weekend)|daily|weekly|hourly|once\s+a\s+(day|week|month)|each\s+(day|week|morning|friday)|at\s+\d{1,2}(:\d{2})?\s*(am|pm)?|\d{1,2}:\d{2}|@\w+)\b/gi;
const WEEKDAY_MAP = {
  sunday: 0,
  monday: 1,
  tuesday: 2,
...
function stripSchedulePhrases(text) {
  return text.replace(SCHEDULE_PHRASE, " ").replace(/\s+/g, " ").trim();
}
...
function parseScheduleIntent(rawInput) {
...
export { formatCronLabel as f, parseScheduleIntent as p };
