...
import { r as resolveHarnessBinding, h as hasRuntimeBindingStamp, a as resolvePlatformAppRoot, b as resolveHostRepoRoot } from './harness-binding_CgEjapvr.mjs';
...
function parseSlashCommand(raw, lifecyclePrefix, cliPrefix) {
  ...
  const lifecycle = lifecyclePrefix.endsWith(":") ? lifecyclePrefix.slice(0, -1) : lifecyclePrefix;
...
function resolveGlobalCommandsWorkspaceRoot(workspaceRoot) {
  const normalizedLocal = resolve(workspaceRoot);
  const explicitOverride = process.env.BUSINESS_PLATFORM_ROOT?.trim() ?? process.env.CONTROL_PLANE_PLATFORM_ROOT?.trim();
  if (explicitOverride) {
    const normalizedOverride = resolve(explicitOverride);
    if (normalizedOverride !== normalizedLocal && hasRuntimeBindingStamp(normalizedOverride)) {
      return normalizedOverride;
    }
  }
  const workspacesMarker = `${sep}workspaces${sep}`;
  if (normalizedLocal.includes(workspacesMarker)) {
    const hostRepo2 = normalizedLocal.split(workspacesMarker)[0];
    const appCandidate = join(hostRepo2, "app");
...
async function scanCommandsDir(commandsDir, scope, lifecyclePrefix, cliPrefix) {
  let entries;
  try {
...
function mergeHarnessCommands(global, local) {
...
async function listHarnessCommands(workspaceRoot) {
  ...
  const globalRoot = workspaceRoot ? resolveGlobalCommandsWorkspaceRoot(workspaceRoot) : null;
  if (!globalRoot) {
...
export { listHarnessCommands as l };
